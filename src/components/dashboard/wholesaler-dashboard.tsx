...
export default function WholesalerDashboard() {
  ...
  const statCards = [
    ...
    },
    {
      ...
      value: stats?.products.verified ?? 0,
      icon: CheckCircle,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <DashboardChart title="Sales Activity" />
        </div>
        ...
        <div className="bg-card rounded-xl border border-border p-5 space-y-4">
          ...
          <div className="space-y-3">
            {[
              { label: "Total in Stock", value: stats?.products.total ?? 0, icon: Package, color: "text-blue-500" },
              { label: "Verified", value: stats?.products.verified ?? 0, icon: CheckCircle, color: "text-emerald-500" },
              { label: "Pending", value: stats?.products.pending ?? 0, icon: Clock, color: "text-amber-500" },
              { label: "Expired", value: stats?.products.expired ?? 0, icon: AlertTriangle, color: "text-red-500" },
              { label: "Low Stock", value: stats?.products.lowStock ?? 0, icon: ShoppingCart, color: "text-orange-500" },
            ].map((item) => {
          ...
          <div className="border-t pt-3 space-y-2">
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">My Suppliers</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-card rounded-xl border border-border p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-semibold text-foreground">Products in Stock</h3>
            <Link href="/products">
          ...
          {!stats?.recentProducts.length ? (
            <p className="text-sm text-muted-foreground py-6 text-center">No products in stock</p>
          ) : (
            <div className="space-y-3">
      ...
      <div className="bg-card rounded-xl border border-border p-5">
        ...
        <div className="flex flex-wrap gap-2">
          <Link href="/products"><Button variant="secondary" size="sm"><Package className="h-4 w-4 mr-1.5" /> My Stock</Button></Link>
